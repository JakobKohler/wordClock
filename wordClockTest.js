...
function clock() {
    ...
    englishFormat = minutesRounded > 30 ? currentHours +1 : currentHours;

    displayedHour = englishFormat > 12 ? englishFormat - 12 : englishFormat;

    minuteCount(currentMinutes);

    if(minutesRounded > 30){
        to(displayedHour, minutesRounded);
    }else if(minutesRounded <= 30 && minutesRounded > 0){
        past(displayedHour,minutesRounded);
    }else{
        oclock(displayedHour);
    }
